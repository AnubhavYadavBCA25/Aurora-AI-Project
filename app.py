...
def ai_recommendation():
    ...
    # Create a form for uploading the dataset and selecting the type of recommendation
    with st.form(key='ai_recommendation'):
    ...
    with st.spinner("Generating Recommendation..."):
        if uploaded_file and type_of_recommendation is not None:
            ...
            st.warning("Please upload a file and select the type of recommendation.")
...
def analysis_report():
    ...
    with st.spinner("Generating Report..."):
        if uploaded_file is not None:
            ...
            # Get the uploaded file name, load the file, clean the data and get report path
            filename = uploaded_file.name
            df = load_file(uploaded_file)
            df = df_cleaning(df)
            report_path = generate_report(df, uploaded_file)

            # Extract the summary and information of the dataset
            summary = df.describe().transpose().to_string()
            df_info = df.info()

            # Provide a predefined prompt for the model
            prompt = f'''Generate a text report for the dataset {filename}. Summary of the dataset is {summary} 
            and Information of dataset is {df_info}.'''
